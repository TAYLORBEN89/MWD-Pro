...
export const MagneticInterference: React.FC = () => {
  ...
  const m = useMemo(() => field(inc, azi, bAxial, bCross), [inc, bAxial, bCross]);
  const st = status(m.dB, m.dDip);

  const scale = 40 / B_REF;
  ...
  const tip =
    Math.abs(m.dAzi) < 0.2 && st.label === 'Pass'
      ? 'Clean field. Btot and dip sit on the IGRF reference. Keep this station.'
      : bAxial > 400
        ? 'Axial steel is adding field along the tool. At high inc that rotates azimuth. Add NMDC or MSA — do not nudge azi by hand.'
        : bCross > 150
          ? 'Cross-axial hotspot. Btot moves and the horizontal field tilts. Rotate and retake; if it walks, steel is too close.'
          : 'Watch band. Take one more station. If Btot stays off, BHA spacing is wrong.';

  return (
    <div className="instrument space-y-2 !p-3">
        ...
        <div className="instrument-title-row">
          ...
          <div>
            ...
            <p className="instrument-subtitle">
              {B_REF.toLocaleString()} nT · dip {DIP_REF}°
            </p>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-white/10 bg-[#07080a] px-2 pt-1 pb-2">
        <svg viewBox="-50 -48 100 96" className="w-full h-[7.25rem]">
          <circle r="44" fill="none" stroke="#27272a" />
          <line x1="-44" x2="44" y1="0" y2="0" stroke="#27272a" />
          <line x1="0" x2="0" y1="-44" y2="44" stroke="#27272a" />
          ...
          <text x="0" y="-36" textAnchor="middle" fill="#71717a" fontSize="7">
            vertical
          </text>
        </svg>
        <div className="flex justify-center gap-4 text-[10px] text-zinc-500 -mt-1">
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {[
          { l: 'Btot', v: `${Math.round(m.btot)}`, d: `${m.dB >= 0 ? '+' : ''}${m.dB.toFixed(0)} nT` },
          { l: 'Dip', v: `${m.dip.toFixed(1)}°`, d: `${m.dDip >= 0 ? '+' : ''}${m.dDip.toFixed(2)}°` },
          { l: 'Azi err', v: `${m.dAzi >= 0 ? '+' : ''}${m.dAzi.toFixed(1)}°`, d: `inc ${inc}°` },
        ].map((row) => (
          <div key={row.l} className="rounded-lg border border-white/10 bg-[#07080a] px-2 py-1.5">
            <p className="label-caps">{row.l}</p>
            <p className="text-[13px] font-mono text-zinc-100 tabular-nums leading-tight">{row.v}</p>
      ...
      <div className="space-y-1.5">
        {[
          ...
          { l: 'Axial', v: bAxial, min: 0, max: 2500, set: setBAxial, fmt: `${bAxial}` },
          { l: 'X-ax', v: bCross, min: 0, max: 800, set: setBCross, fmt: `${bCross}` },
        ].map((s) => (
          <label key={s.l} className="flex items-center gap-2">
            <span className="label-caps w-9 shrink-0">{s.l}</span>
            <input
              type="range"
              min={s.min}
              max={s.max}
            ...
            />
            <span className="w-10 text-right text-[11px] font-mono text-zinc-300 tabular-nums">{s.fmt}</span>
          </label>
        ))}
      </div>

      <div className="instrument-tip !p-2">
